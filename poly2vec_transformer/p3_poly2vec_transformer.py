...
sys.path.append(os.path.abspath("../03_selectivity/"))
...
def convert_data(data_loader):
    ...
    for batch_idx in progress_bar:
        ...
        features = torch.zeros((y.shape[0], 1024, 8))
        for i in range(len(indexes)):
            ...
            points[i, :, :] = _points
            # _points = torch.zeros((1024, 5, 2))
            # _points[:, 0, :] = _features[:, [0,1]]
            # _points[:, 1, :] = _features[:, [0,3]]
            # _points[:, 2, :] = _features[:, [2,3]]
            # _points[:, 3, :] = _features[:, [2,1]]
            # _points[:, 4, :] = _features[:, [0,1]]
            # points[i, :, :] = _points
...
class SimpleTransformerModel(nn.Module):
    def __init__(self, geometry_model, input_dim=40, output_dim=1, max_seq_len=1024, nhead=4, num_layers=4, dim_feedforward=128):
    ...
    def forward(self, geometries, lengths, dataset_type, features):
        x = torch.zeros((geometries.shape[0], 1024, 40)).to(device)
        for i in range(geometries.shape[0]):
            x[i, :, :32] = self.geometry_encoder(geometries[i], lengths[i], dataset_type)
            x[i, :, 32:40] = features[i, :, :]
...
def train_step(model, optimizer, epoch, train_data):
    ...
    for batch_idx in progress_bar:
        ...
        optimizer.zero_grad()
        prediction = model(points.to(device), lengths, "points", features.to(device))
...
def val_step(model, epoch, val_data, label, best_wmape):
    ...
    if wmape < best_wmape:
        ...
        df[f"predicted"] = predictions[:, :].cpu().numpy().flatten() 
        best_wmape = wmape
        cur_path = output_folder + "/%s_%.6f.csv" % (label, best_wmape)
        checkpoint_path = output_folder + "/%s_%.6f.ckpt" % (label, best_wmape)
...
if __name__ == "__main__":
    ...
    for i in range(1,max_epochs+1):
            ...
            best_wmape = val_step(model, i, val_data, label, best_wmape)
