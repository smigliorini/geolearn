...
def custom_collate_fn(batch):
    ...
    for item in batch:
        ...
        params += [list(item[1][0][1:])]*len(item[3])
        i+=1
    ...
    return tensor1_batch, tensor2_batch, tensor3_batch, tensor4_batch, tensor5_batch, batch_ids, params
...
if __name__ == "__main__":
    ...
    for batch_idx in progress_bar:
        points, features, y, all_y, nearest_indices, batch_ids, params = next(iter1)
        actual = all_y if not isinstance(actual, np.ndarray) else np.concatenate((actual, all_y))
        all_params = params if not isinstance(all_params, np.ndarray) else np.concatenate((all_params, params))
    ...
    # df["param"] = all_params
    df["params_0"] = all_params[:, 0]
    df["params_1"] = all_params[:, 1]
    df["params_2"] = all_params[:, 2]
    df.to_csv(results_folder + '/poly2vec_param.csv', index=False)
